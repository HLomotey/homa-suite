...
CREATE TABLE month_end_reports (
  ...
  key_wins TEXT [],
  -- Hotel Occupancy tab
  occupancy_start_pct DECIMAL(5, 3) CHECK (
    occupancy_start_pct >= 0
    AND occupancy_start_pct <= 100
  ),
  occupancy_end_pct DECIMAL(5, 3) CHECK (
    occupancy_end_pct >= 0
    AND occupancy_end_pct <= 100
  ),
  avg_occupancy_pct DECIMAL(5, 3) CHECK (
    avg_occupancy_pct >= 0
    AND avg_occupancy_pct <= 100
  ),
  occupancy_notes TEXT,
  -- Guest Room Cleanliness tab
  cleanliness_score DECIMAL(4, 3) CHECK (
    cleanliness_score >= 0
    AND cleanliness_score <= 1
  ),
  ...
  CONSTRAINT month_end_reports_headline_length CHECK (
    char_length(headline) >= 10
    AND char_length(headline) <= 120
  ),
  CONSTRAINT month_end_reports_narrative_length CHECK (
    char_length(narrative) >= 50
    AND char_length(narrative) <= 3000
  )
);
...
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_reports'
    AND policyname = 'Users can view month end reports'
) THEN EXECUTE 'CREATE POLICY "Users can view month end reports" ON month_end_reports FOR SELECT USING (auth.role() = ''authenticated'')';
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_reports'
    AND policyname = 'Users can insert month end reports'
) THEN EXECUTE 'CREATE POLICY "Users can insert month end reports" ON month_end_reports FOR INSERT WITH CHECK (auth.role() = ''authenticated'')';
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_reports'
    AND policyname = 'Users can update month end reports'
) THEN EXECUTE 'CREATE POLICY "Users can update month end reports" ON month_end_reports FOR UPDATE USING (auth.role() = ''authenticated'')';
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_reports'
    AND policyname = 'Users can delete month end reports'
) THEN EXECUTE 'CREATE POLICY "Users can delete month end reports" ON month_end_reports FOR DELETE USING (auth.role() = ''authenticated'')';
END IF;
END $$;
...
DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_report_groups'
    AND policyname = 'Users can view report groups'
) THEN EXECUTE 'CREATE POLICY "Users can view report groups" ON month_end_report_groups FOR SELECT USING (auth.role() = ''authenticated'')';
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_report_groups'
    AND policyname = 'Users can insert report groups'
) THEN EXECUTE 'CREATE POLICY "Users can insert report groups" ON month_end_report_groups FOR INSERT WITH CHECK (auth.role() = ''authenticated'')';
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_report_groups'
    AND policyname = 'Users can update report groups'
) THEN EXECUTE 'CREATE POLICY "Users can update report groups" ON month_end_report_groups FOR UPDATE USING (auth.role() = ''authenticated'')';
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_report_groups'
    AND policyname = 'Users can delete report groups'
) THEN EXECUTE 'CREATE POLICY "Users can delete report groups" ON month_end_report_groups FOR DELETE USING (auth.role() = ''authenticated'')';
END IF;
END $$;
...
DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_report_action_items'
    AND policyname = 'Users can view action items'
) THEN EXECUTE 'CREATE POLICY "Users can view action items" ON month_end_report_action_items FOR SELECT USING (auth.role() = ''authenticated'')';
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_report_action_items'
    AND policyname = 'Users can insert action items'
) THEN EXECUTE 'CREATE POLICY "Users can insert action items" ON month_end_report_action_items FOR INSERT WITH CHECK (auth.role() = ''authenticated'')';
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_report_action_items'
    AND policyname = 'Users can update action items'
) THEN EXECUTE 'CREATE POLICY "Users can update action items" ON month_end_report_action_items FOR UPDATE USING (auth.role() = ''authenticated'')';
END IF;
IF NOT EXISTS (
  SELECT 1
  FROM pg_policies
  WHERE tablename = 'month_end_report_action_items'
    AND policyname = 'Users can delete action items'
) THEN EXECUTE 'CREATE POLICY "Users can delete action items" ON month_end_report_action_items FOR DELETE USING (auth.role() = ''authenticated'')';
END IF;
END $$;
-- Create trigger functions and triggers for timestamp updates
CREATE OR REPLACE FUNCTION update_month_end_reports_timestamp() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
NEW.updated_by = auth.uid();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER update_month_end_reports_timestamp BEFORE
UPDATE ON month_end_reports FOR EACH ROW EXECUTE FUNCTION update_month_end_reports_timestamp();
-- Create trigger to set created_by on insert
CREATE OR REPLACE FUNCTION set_month_end_reports_created_by() RETURNS TRIGGER AS $$ BEGIN NEW.created_by = auth.uid();
NEW.prepared_by = COALESCE(NEW.prepared_by, auth.uid());
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER set_month_end_reports_created_by BEFORE
INSERT ON month_end_reports FOR EACH ROW EXECUTE FUNCTION set_month_end_reports_created_by();
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER update_month_end_report_groups_timestamp BEFORE
UPDATE ON month_end_report_groups FOR EACH ROW EXECUTE FUNCTION update_timestamp();
CREATE TRIGGER update_month_end_report_action_items_timestamp BEFORE
UPDATE ON month_end_report_action_items FOR EACH ROW EXECUTE FUNCTION update_timestamp();
-- Add comments on tables and columns for better documentation
COMMENT ON TABLE month_end_reports IS 'Operations Call Meeting Report capturing recurring indicators and operational data';
