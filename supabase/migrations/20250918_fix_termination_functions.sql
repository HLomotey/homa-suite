-- Fix the termination request functions with proper delimiters

-- Create enums for termination request fields if they don't exist
DO $$ 
BEGIN
...
-- Create termination_requests table if it doesn't exist
...
-- Drop and recreate submit_termination_request function
DROP FUNCTION IF EXISTS submit_termination_request(UUID, UUID, DATE, DATE, separation_type, TEXT, BOOLEAN, direct_deposit_instruction, TEXT);
...
-- Drop and recreate process_termination_request function
DROP FUNCTION IF EXISTS process_termination_request(UUID, termination_status, TEXT);
