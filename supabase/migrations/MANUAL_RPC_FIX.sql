...
-- Go to: https://supabase.com/dashboard/project/vconsodaevbqavtijaby/sql/new
...
BEGIN
    ...
    RETURN QUERY
    SELECT 
        SPLIT_PART(p.title, ':', 1) as resource,
        SPLIT_PART(p.title, ':', 2) as action,
        p.title as permission_key
    FROM permissions p
