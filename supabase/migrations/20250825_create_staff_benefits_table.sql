...
CREATE TYPE benefit_type AS ENUM ('housing', 'transportation')

-- Create benefit_status enum  
...
CREATE TABLE staff_benefits (
  ...
  CONSTRAINT staff_benefits_effective_expiry_check CHECK (expiry_date IS NULL OR expiry_date > effective_date)
);
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'staff_benefits' AND policyname = 'Users can view staff benefits') THEN
    EXECUTE 'CREATE POLICY "Users can view staff benefits" ON staff_benefits FOR SELECT USING (auth.role() = ''authenticated'')';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'staff_benefits' AND policyname = 'Users can insert staff benefits') THEN
    EXECUTE 'CREATE POLICY "Users can insert staff benefits" ON staff_benefits FOR INSERT WITH CHECK (auth.role() = ''authenticated'')';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'staff_benefits' AND policyname = 'Users can update staff benefits') THEN
    EXECUTE 'CREATE POLICY "Users can update staff benefits" ON staff_benefits FOR UPDATE USING (auth.role() = ''authenticated'')';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'staff_benefits' AND policyname = 'Users can delete staff benefits') THEN
    EXECUTE 'CREATE POLICY "Users can delete staff benefits" ON staff_benefits FOR DELETE USING (auth.role() = ''authenticated'')';
  END IF;
END
$$;

-- Create trigger to update timestamps and user tracking
CREATE OR REPLACE FUNCTION update_staff_benefits_timestamp()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
...
CREATE TRIGGER update_staff_benefits_timestamp
BEFORE UPDATE ON staff_benefits
FOR EACH ROW EXECUTE FUNCTION update_staff_benefits_timestamp();

-- Create trigger to set created_by on insert
CREATE OR REPLACE FUNCTION set_staff_benefits_created_by()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER set_staff_benefits_created_by
BEFORE INSERT ON staff_benefits
FOR EACH ROW EXECUTE FUNCTION set_staff_benefits_created_by();

-- Add comments on table and columns for better documentation
COMMENT ON TABLE staff_benefits IS 'Staff benefits tracking housing and transportation requirements';
