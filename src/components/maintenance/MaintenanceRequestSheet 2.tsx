...
export function MaintenanceRequestSheet({ children, onSuccess, open, onOpenChange }: MaintenanceRequestSheetProps) {
  ...
  const resetForm = () => {
    form.reset();
    setImages([]);
    // Revoke all image URLs to avoid memory leaks
  ...
  const onSubmit = (values: FormValues) => {
    ...
    const requestData: Omit<FrontendMaintenanceRequest, "id"> = {
      ...
      tenantId: currentUser?.user?.id || null,
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mt-6">
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
                    <div className="space-y-0.5">
                      <FormLabel className="text-sm font-medium">Emergency Issue</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
                    <div className="space-y-0.5">
                      <FormLabel className="text-sm font-medium">Permission to Enter</FormLabel>
                      <FormDescription className="text-xs">
