...
export const StaffBenefitsList: React.FC<StaffBenefitsListProps> = ({
  ...
  const filteredBenefits = Object.values(groupedBenefits).filter(
    (staffBenefits) => {
      const matchesSearch =
        staffBenefits.staff_name
          ?.toLowerCase()
          .includes(searchQuery.toLowerCase()) ||
        staffBenefits.staff_email
          ?.toLowerCase()
          .includes(searchQuery.toLowerCase()) ||
        staffBenefits.staff_department
          ?.toLowerCase()
          .includes(searchQuery.toLowerCase()) ||
        staffBenefits.notes?.toLowerCase().includes(searchQuery.toLowerCase());

      const matchesStatus =
        statusFilter === "all" || staffBenefits.statuses.includes(statusFilter);
      const matchesType =
        typeFilter === "all" ||
  ...
  const handleDelete = async (benefit: FrontendStaffBenefit) => {
    if (
      window.confirm(
        `Are you sure you want to delete this benefit for ${benefit.staff_name}?`
      )
    ) {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Housing & Transportation</CardTitle>
              <CardDescription>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-4">
            ...
            <Select
              value={statusFilter}
              onValueChange={(value) =>
                setStatusFilter(value as BenefitStatus | "all")
              }
            >
            ...
            <Select
              value={typeFilter}
              onValueChange={(value) =>
                setTypeFilter(value as BenefitType | "all")
              }
            >
              <SelectTrigger className="w-[180px]">
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center space-x-2">
                      <div className="flex space-x-1">
                        {staffBenefits.benefit_types.includes("housing") &&
                          getBenefitTypeIcon("housing")}
                        {staffBenefits.benefit_types.includes(
                          "transportation"
                        ) && getBenefitTypeIcon("transportation")}
                      </div>
                      <div>
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="text-right">
                      ...
                      <div className="text-sm text-muted-foreground">
                        Benefit Type
                        {staffBenefits.benefit_types.length > 1 ? "s" : ""}
                      </div>
                    </div>
                      ...
                      <div className="flex flex-col space-y-1">
                        {staffBenefits.benefits.map((benefit, index) => (
                          ...
                          >
                            ...
                            <Badge
                              variant={getStatusBadgeVariant(benefit.status)}
                            >
                    ...
                    <div className="text-right text-sm text-muted-foreground">
                      <div>
                        Effective:{" "}
                        {format(
                          new Date(staffBenefits.effective_date),
                          "MMM dd, yyyy"
                        )}
                      </div>
                      {staffBenefits.benefits[0]?.expiry_date && (
                        <div>
                          Expires:{" "}
                          {format(
                            new Date(staffBenefits.benefits[0].expiry_date),
                            "MMM dd, yyyy"
                          )}
                        </div>
                      )}
                        ...
                        {staffBenefits.benefits.map((benefit) => (
                          <div key={benefit.id}>
                            ...
                            >
                              ...
                              View{" "}
                              {benefit.benefit_type.charAt(0).toUpperCase() +
                                benefit.benefit_type.slice(1)}
                            </DropdownMenuItem>
                            <DropdownMenuItem
