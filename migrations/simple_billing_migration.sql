-- Simple Migration to add missing billing columns and tables
-- Created: 2025-10-05
-- This version removes all foreign key constraints to avoid reference errors
...
-- Create billing_deductions table (no foreign keys)
...
-- Create flight_agreements table (no foreign keys)
...
-- Create flight_agreement_deductions table (no foreign keys)
...
CREATE INDEX IF NOT EXISTS idx_flight_deductions_staff_id ON flight_agreement_deductions(staff_id);
